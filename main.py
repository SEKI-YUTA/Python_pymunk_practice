...
def create_structure(space, width, height):
    BROWN = (139, 69, 19, 100)
    rects = [
        [(600, 120), (40, 200), BROWN, 100],
        [(900, 120), (40, 200), BROWN, 100],
        [(750, 240), (340, 40), BROWN, 150]
    ]
    for pos, size, color, mass in rects:
        body = pymunk.Body()
        body.position = pos
        shape = pymunk.Poly.create_box(body, size, radius=5)
        shape.color = color
        shape.mass = mass
        shape.elasticity = 0.4
        shape.friction = 0.4
...
def create_swinging_ball(space):
    ...
    rotation_center_body.position = (300, 570)
    body = pymunk.Body()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if not ball:
                    ...
                    print 'pressed pos: {0}'.format(pressed_pos)
